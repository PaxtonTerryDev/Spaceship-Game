...
func _handle_ship_rotation() -> void:
	...
	ship.movement_component.target_rotation = direction_to_mouse.angle()
...
func _handle_ship_thruster_inputs() -> void:
	...
	if Input.is_action_pressed("ship_stern_thrusters"):
		...
		ship.movement_component.engage_thruster(Ship.ShipSide.STERN, action_strength)
	...
	if Input.is_action_pressed("ship_bow_thrusters"):
		...
		ship.movement_component.engage_thruster(Ship.ShipSide.BOW, action_strength)
	...
	if Input.is_action_pressed("ship_port_thrusters"):
		...
		ship.movement_component.engage_thruster(Ship.ShipSide.PORT, action_strength)
	...
	if Input.is_action_pressed("ship_starboard_thrusters"):
		...
		ship.movement_component.engage_thruster(Ship.ShipSide.STARBOARD, action_strength)

func 	_handle_ship_weapon_inputs() -> void:
	if Input.is_action_just_pressed("ship_fire_active_weapon_group"):
		ship.weapon_component.fire_active_group()
